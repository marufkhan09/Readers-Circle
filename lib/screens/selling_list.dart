...
class SellingListScreen extends StatefulWidget {
  const SellingListScreen({super.key});

  @override
  State<SellingListScreen> createState() => _SellingListScreenState();
}

class _SellingListScreenState extends State<SellingListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: context.watch<BookProvider>().booksForSaleLoaded
          ? ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount:
                  context.watch<BookProvider>().booksForSale.data!.length,
              itemBuilder: (context, index) {
                final book =
                    context.watch<BookProvider>().booksForSale.data![index];
                return InkWell(
                  onTap: () {
